...
def get_neighbors(current, config, ox, oy, kd_tree):
    for steer in calc_motion_inputs():
        node = calc_next_node(current, steer, config, ox, oy, kd_tree)
        if node and verify_index(node, config):
            yield node


def calc_next_node(current, steer, config, ox, oy, kd_tree):
    x, y, yaw= current.x_list[-1], current.y_list[-1], current.yaw_list[-1]

    arc_l = XY_GRID_RESOLUTION * 1.5
    x_list, y_list, yaw_list= [], [], []
    for _ in np.arange(0, arc_l, MOTION_RESOLUTION):
        x, y, yaw = move(x, y, yaw, 0.1, steer)
        x_list.append(x)
        y_list.append(y)
        yaw_list.append(yaw)
...
#         start, goal, ox, oy, XY_GRID_RESOLUTION)
...
def save_curr_to_csv(loc, filename='curr_hybrid.txt'):
...
class Agent():
    ...
    def run_step(self, filtered_obstacles, waypoints, vel, transform, boundary):
        ...
        ox, oy = self.process_boundaries(boundary)
        ...
        mid = (outer_border+inner_border)/2
        final = mid[-1]
        goal = []
        for i in final:
            goal.append(i)
        ...
        # obs = []
        # obs = zip(*obstacle_list) if obstacle_list else ([], [])
        
        # print(type(obs))
        print("start : ", start)
        print("goal : ", goal)
        path = hybrid_a_star_planning(start, list(goal), ox, oy, XY_GRID_RESOLUTION, YAW_GRID_RESOLUTION)
        # print(type(path))
        # print(path.x_list,path.y_list
        ...
        try:
            wp = list(zip(path.x_list,path.y_list, path.yaw_list))
            target_velocity, steer, brake = self.control(curr_x, curr_y, current_velocity ,curr_yaw, wp,filtered_obstacles)
        except:
            target_velocity, steer, brake = self.control(curr_x, curr_y, current_velocity ,curr_yaw, waypoints,filtered_obstacles)
            print('w')
