...
inner_border = inner_border[0:725]
outer_border = outer_border[0:725]
midpoints = (inner_border + outer_border) / 2
...
divisions = 7
border_threshold = 3  # This should be set according to the minimum allowed distance to the border
...
plt.scatter(inner_border[:, 0], inner_border[:, 1], c='black', marker='o', label='Inner Border')
plt.scatter(outer_border[:, 0], outer_border[:, 1], c='black', marker='o', label='Outer Border')
...
#         plt.scatter(waypoints_grid[i, j, 0], waypoints_grid[i, j, 1], c='blue', s=5,)  # s is the size of points

# # Plotting the optimal path
# plt.plot(optimal_path[:, 0], optimal_path[:, 1], 'r-', label='Dynamic Programming Path')
plt.plot(curr_arr[:, 0], curr_arr[:,1], 'b-',label='Spline Interpolation Star Path')
