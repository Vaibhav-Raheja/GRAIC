...
class Config:
    ...
    def __init__(self):
        ...
        self.robot_width = 2.875  # [m] for collision check
        self.robot_length = 4.0  # [m] for collision check
        # obstacles [x(m) y(m), ....]
    ...
    def process_boundaries(self, boundary):
        ...
        for boundary_side in boundary:  # boundary_side is either left_boundary or right_boundary
            for waypoint in boundary_side:
                ...
                obstacle = (x, y)  # Example radius of 0.5 meters
                obstacles.append(obstacle)
        return obstacles
    def run_step(self, filtered_obstacles, waypoints, vel, transform, boundary):
        """
        Execute one step of navigation.
        :param filtered_obstacles: Locations of obstacles.
        :param waypoints: List of waypoints to follow.
        :param vel: Current velocity of the vehicle.
        :param transform: Current transform (position and orientation) of the vehicle.
        :param boundary: Boundary of the navigation area.
        :param config: Configuration parameters for DWA.
        :param goal: The goal position (x, y).
        :return: Carla VehicleControl object.
        """
        # Convert Carla velocity to numpy array
        current_velocity = self.get_speed(vel)
        current_speed = np.linalg.norm(current_velocity)

        # Convert Carla transform to DWA state
        ...
        x = np.array([curr_x, curr_y, curr_yaw, current_speed, 0.0])

        # Convert filtered_obstacles to numpy array
        # waypoints_inside_boundary = [wp for wp in waypoints if is_inside_boundary(wp, boundary)]
        # obstacles_inside_boundary = [ob for ob in filtered_obstacles if is_inside_boundary(ob, boundary)]
        # combined_obstacles = filtered_obstacles + boundary_points(boundary)
        ob = np.array(self.process_boundaries(boundary))

        # Calculate control using DWA
        goal = waypoints[10][0], waypoints[10][1]
        u, trajectory = dwa_control(x, self, goal, ob)
        target_speed = u[0]
        target_yaw_rate = u[1]

        # Convert target speed and yaw rate to CARLA control commands
        control = carla.VehicleControl()
        control.throttle = max(0, target_speed)  # Throttle if speed is positive
        control.brake = 0.0  # Brake if speed is negative
        control.steer = np.clip(target_yaw_rate / config.max_yaw_rate, -1.0, 1.0)
